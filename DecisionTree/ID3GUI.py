...
import sys
import ID3, DataFrameModel, make_folder
import pandas as pd
import pprint
import json
...
class MyWindow(QtWidgets.QMainWindow):
    ...
    def on_Import_clicked(self):
        ...
        if dataPath:
            ...
            # tree = ID3.buildTree(model._dataframe)
            # pprint.pprint(tree)
        # # Tạo predict. load len QtableviewPredict
        # make_folder.create_folder('./Predict/')
        # predictDir = './Predict/'
        # filePredictName = 'predict_{}.csv'.format(self.labelInputName.text())
        # with open(predictDir + filePredictName, 'w', newline='') as csvFile:
        #     csvwriter = csv.writer(csvFile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
        #     csvwriter.writerows([attributes])
        #     csvwriter.writerows([['']*len(attributes)])
        
        # with open(predictDir + filePredictName, 'a', newline='') as csvFile:
        #     dfPredict = pd.read_csv(predictDir + filePredictName, encoding='utf-8')
        #     modelPredict = DataFrameModel.DataFrameModel(dfPredict)
        # self.tableViewPredict.setModel(modelPredict)
